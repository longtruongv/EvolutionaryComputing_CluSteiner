...
def crossover_sbx(indi1:Individual, indi2:Individual, rand:Random):
    chromosome_len = indi1.length
    ...
    u = rand.random()
    if (u <= 0.5):
        beta = (2 * u) ** (1.0 / (1 + SBX_COEF))
    else:
        beta = (1 / (2 * (1 - u))) ** (1.0 / (1 + SBX_COEF))

    for i in range(chromosome_len):
        gene1 = 0.5 * ( (1 + beta) * indi1.get(i) + (1 - beta) * indi2.get(i) )
        child1.set(i, gene1)

        gene2 = 0.5 * ( (1 - beta) * indi1.get(i) + (1 + beta) * indi2.get(i) )
        child2.set(i, gene2)
